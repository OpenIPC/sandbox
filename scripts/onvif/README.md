...
From this point forward it is expected that you have successfully built, installed and tested your ability to make and deploy the firmware for your camera.
...
At the time of testing this setup I couldn't see an obvious way to get Majestic to use our onvif solution, instead of its own, and so we need to run a parallel httpd instance on a different port to make it work seamlessly (we are still using the same web content location though)

To achieve this we are going to create a script to load the httpd daemon on port 8080 in the same way the other modules are started using init.d.
...
It is recommend to enable the onvif username/password security by creating **viewer** user and editing the onvif.conf file located at /etc/onvif.conf file by entering
...
Use the arrows key to navigate to the password setting, press the insert key to enable editing and then update your password. To save press the ESC key and then press :wq which is the command to write and then quit.
